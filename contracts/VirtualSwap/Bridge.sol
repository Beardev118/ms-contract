 ...
 *
 * 1) tokenToSynth
 * Swaps a supported token in a metal pool to any synthetic asset (e.g. tBTC -> sAAVE).
 *
 * 2) synthToToken
 * Swaps any synthetic asset to a suported token in a metal pool (e.g. sDEFI -> USDC).
 *
 * 3) tokenToToken
 * Swaps a supported token in a metal pool to one in another pool (e.g. renBTC -> DAI).
 *
...
contract Bridge is ERC721 {
    ...
    constructor(address synthSwapperAddress)
        public
        ERC721("metal Cross-Asset Swap", "metalSynthSwap")
    {
    ...
    /**
     * @notice Settles and withdraws the synthetic asset without swapping it to a token in a metal pool. Only the owner
     ...
     * @param swap the address of a metal pool to use to swap the given token to a bridging synth
     ...
     * @param swaps the addresses of the two metal pools used to do the cross-asset swap
    ...
    /**
     * @notice Initiates a cross-asset swap from a token in one metal pool to one in another. The caller will receive
     * an ERC721 token representing their ownership of the pending cross-asset swap.
     * @param swaps the addresses of the two metal pools used to do the cross-asset swap
     * @param tokenFromIndex the index of the token in the first `swaps` pool to swap from
     * @param tokenToIndex the index of the token in the second `swaps` pool to swap to
