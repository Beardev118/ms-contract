...
describe("GeneralizedSwapMigrator", () => {
  ...
  const setupTest = deployments.createFixture(
    async ({ deployments, ethers }) => {
      ...
      oldUSDPool = await ethers.getContract("metalUSDPool")
      oldUSDPoolLPToken = await ethers.getContract("metalUSDPoolLPToken")
      newUSDPool = await ethers.getContract("metalUSDPoolV2")
      newUSDPoolLPToken = await ethers.getContract("metalUSDPoolV2LPToken")
