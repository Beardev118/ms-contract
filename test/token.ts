...
describe("Token", () => {
  ...
  describe("minting", () => {
    it("Successfully mints to appropriate addresses and vestings", async () => {
      ...
      expect(await metalToken.balanceOf(await signers[10].getAddress())).to.eq(
        0,
      )
      expect(await metalToken.balanceOf(await signers[11].getAddress())).to.eq(
        0,
      )
      expect(await metalToken.balanceOf(await signers[12].getAddress())).to.eq(
        0,
      )
      expect(await metalToken.balanceOf(vestingContracts[0].address)).to.eq(
        BIG_NUMBER_1E18.mul(2e8),
      )
      expect(await metalToken.balanceOf(vestingContracts[1].address)).to.eq(
        BIG_NUMBER_1E18.mul(2e8),
      )
      expect(await metalToken.balanceOf(vestingContracts[2].address)).to.eq(
        BIG_NUMBER_1E18.mul(2e8),
      )

      // Non-vesting addresses
      expect(await metalToken.balanceOf(await signers[13].getAddress())).to.eq(
        BIG_NUMBER_1E18.mul(2e8),
      )
      expect(await metalToken.balanceOf(await signers[14].getAddress())).to.eq(
        BIG_NUMBER_1E18.mul(2e8),
      )
    })
  })
  ...
  describe("totalSupply", () => {
    it("Successfully mints max supply (1e9 with 1e18 decimals) on deployment", async () => {
      expect(await metalToken.totalSupply()).to.eq(BIG_NUMBER_1E18.mul(1e9))
    })
  })

  describe("govCanUnpauseAfter", () => {
    it("Successfully sets govCanUnpauseAfter to be in the future on deployment", async () => {
      expect(await metalToken.govCanUnpauseAfter()).to.eq(
  ...
  describe("anyoneCanUnpauseAfter", () => {
    it("Successfully sets anyoneCanUnpauseAfter to be 1 year after the deployment", async () => {
      expect(await metalToken.anyoneCanUnpauseAfter()).to.eq(
  ...
  describe("governance", () => {
    it("Successfully sets the governance address on deployment", async () => {
      expect(await metalToken.governance()).to.eq(governanceAddress)
    })
  })
  ...
  describe("transfer", () => {
    it("Successfully transfers from an allowed address when paused", async () => {
      ...
      expect(await metalToken.balanceOf(deployerAddress)).to.eq(
  ...
  describe("enableTransfer", () => {
    it("Reverts when governance attempts to unpause before govCanUnpauseAfter", async () => {
      expect(await metalToken.govCanUnpauseAfter()).to.gt(
        await getCurrentBlockTimestamp(),
      )
      await expect(
        metalToken.connect(governance).enableTransfer(),
    ...
    it("Reverts when non-governance attempts to unpause after govCanUnpauseAfter", async () => {
      await setTimestamp(
        (await metalToken.govCanUnpauseAfter()).add(1).toNumber(),
      )
      expect(await metalToken.govCanUnpauseAfter()).to.lte(
        await getCurrentBlockTimestamp(),
      )

      await expect(
        metalToken.connect(deployer).enableTransfer(),
      ).to.be.revertedWith("SDL: cannot enable transfer yet")
    })
    ...
    it("Succeeds when governance attempts to unpause after govCanUnpauseAfter", async () => {
      ...
      await metalToken.connect(governance).enableTransfer()
      expect(await metalToken.paused()).to.be.false
    })

    it("Succeeds when non-governance attempts to unpause after anyoneCanUnpauseAfter", async () => {
      await setTimestamp(
        (await metalToken.anyoneCanUnpauseAfter()).add(1).toNumber(),
      )
      expect(await metalToken.anyoneCanUnpauseAfter()).to.lte(
        await getCurrentBlockTimestamp(),
      )

      await metalToken.enableTransfer()
      expect(await metalToken.paused()).to.be.false
    })
    ...
    describe("transfer after enableTransfer", () => {
      ...
      it("Succeeds when transferring between not-allowed addresses", async () => {
        ...
        expect(
          await metalToken.balanceOf(await signers[16].getAddress()),
  ...
  describe("addToAllowedList", () => {
    it("Succeeds to add an address to the allowed list", async () => {
      ...
      await metalToken.connect(governance).addToAllowedList([deployerAddress])
    ...
    it("Reverts when called by non-governance", async () => {
      await expect(
        metalToken.connect(deployer).addToAllowedList([deployerAddress]),
  ...
  describe("changeGovernance & acceptGovernance", () => {
    it("Reverts when called by other than the governance", async () => {
      await expect(
        metalToken.connect(deployer).changeGovernance(deployerAddress),
      ).to.be.revertedWith("only governance can perform this action")
    })

    it("Succeeds to change governance", async () => {
      await metalToken.connect(governance).changeGovernance(deployerAddress)
      expect(await metalToken.governance()).to.be.eq(governanceAddress)
      await metalToken.connect(deployer).acceptGovernance()
      expect(await metalToken.governance()).to.be.eq(deployerAddress)
      expect(await metalToken.pendingGovernance()).to.be.eq(ZERO_ADDRESS)
    })

    it("Reverts when accepting governance when changeGovernance is not called before", async () => {
      await expect(
        metalToken.connect(deployer).acceptGovernance(),
    ...
    it("Reverts when accepting governance when called by other than pendingGovernance", async () => {
      await metalToken.connect(governance).changeGovernance(deployerAddress)
      expect(await metalToken.pendingGovernance()).to.be.eq(deployerAddress)
      await expect(
        metalToken.connect(signers[13]).acceptGovernance(),
  ...
  describe("rescueToken", () => {
    ...
    it("Successfully rescues ERC20", async () => {
      ...
      await dummyToken.mint(metalToken.address, BIG_NUMBER_1E18.mul(10000))

      expect(await dummyToken.balanceOf(metalToken.address)).to.eq(
