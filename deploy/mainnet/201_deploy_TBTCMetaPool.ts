...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  const metalTBTCMetaPool = await getOrNull("metalTBTCMetaPool")
  if (metalTBTCMetaPool) {
    log(`reusing "metalTBTCMetaPool" at ${metalTBTCMetaPool.address}`)
  } else {
    ...
    await save("metalTBTCMetaPool", {
      abi: (await get("metalSUSDMetaPool")).abi,
      address: btcSwapAddress,
    })
  }

  const lpTokenAddress = (await read("metalTBTCMetaPool", "swapStorage"))
    .lpToken
  log(`metal tBTC v2 MetaSwap LP Token at ${lpTokenAddress}`)

  await save("metalTBTCMetaPoolLPToken", {
