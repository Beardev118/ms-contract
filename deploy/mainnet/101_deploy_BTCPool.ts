...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  const LP_TOKEN_NAME = "metal tBTC/WBTC/renBTC/sBTC"
  const LP_TOKEN_SYMBOL = "metalTWRenSBTC"
  const INITIAL_A = 200
  ...
  const lpTokenAddress = (await read("metalBTCPool", "swapStorage")).lpToken
  log(`BTC pool LP Token at ${lpTokenAddress}`)

  await save("metalBTCPoolLPToken", {
