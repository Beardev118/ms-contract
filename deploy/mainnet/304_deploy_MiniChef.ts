...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  if ((await getOrNull("MiniChefV2")) == null) {
    ...
    const batchCall = [
      await minichef.populateTransaction.setmetalPerSecond(lmRewardsPerSecond),
      ...
      await minichef.populateTransaction.add(
        0,
        (
          await get("metalALETHPoolLPToken")
        ).address,
        "0x0000000000000000000000000000000000000000",
      ),
      await minichef.populateTransaction.add(
      ...
      await minichef.populateTransaction.add(
        0,
        (
          await get("metalUSDPoolV2LPToken")
        ).address,
        "0x0000000000000000000000000000000000000000",
      ),
      await minichef.populateTransaction.add(
        0,
        (
          await get("metalBTCPoolV2LPToken")
        ).address,
        "0x0000000000000000000000000000000000000000",
      ),
    ]
