...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  const metalALETHPool = await getOrNull("metalALETHPool")
  if (metalALETHPool) {
    log(`reusing "metalALETHPool" at ${metalALETHPool.address}`)
  } else {
    ...
    const LP_TOKEN_NAME = "metal WETH/alETH/sETH"
    const LP_TOKEN_SYMBOL = "metalalETH"
    const INITIAL_A = 60
    ...
    await save("metalALETHPool", {
      abi: (await get("SwapFlashLoan")).abi,
      address: alETHSwapAddress,
    })
  }

  const lpTokenAddress = (await read("metalALETHPool", "swapStorage")).lpToken
  log(`alETH pool LP Token at ${lpTokenAddress}`)

  await save("metalALETHPoolLPToken", {
