...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  const metalUSDPool = await getOrNull("metalUSDPool")
  if (metalUSDPool) {
    log(`reusing "metalUSDPool" at ${metalUSDPool.address}`)
  } else {
    ...
    const LP_TOKEN_NAME = "metal DAI/USDC/USDT"
    const LP_TOKEN_SYMBOL = "metalUSD"
    const INITIAL_A = 200
    ...
    await save("metalUSDPool", {
      abi: (await get("SwapFlashLoanV1")).abi,
      address: usdSwapAddress,
    })
  }

  const lpTokenAddress = (await read("metalUSDPool", "swapStorage")).lpToken
  log(`USD pool LP Token at ${lpTokenAddress}`)

  await save("metalUSDPoolLPToken", {
