...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  const metalD4Pool = await getOrNull("metalD4Pool")
  if (metalD4Pool) {
    log(`reusing "metalD4Pool" at ${metalD4Pool.address}`)
  } else {
    ...
    const LP_TOKEN_NAME = "metal alUSD/FEI/FRAX/LUSD"
    const LP_TOKEN_SYMBOL = "metalD4"
    const INITIAL_A = 60
    ...
    await save("metalD4Pool", {
      abi: (await get("SwapFlashLoan")).abi,
      address: D4SwapAddress,
    })
  }

  const lpTokenAddress = (await read("metalD4Pool", "swapStorage")).lpToken
  log(`D4 pool LP Token at ${lpTokenAddress}`)

  await save("metalD4PoolLPToken", {
